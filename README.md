# Ryan Matthews

[![Download CV](https://img.shields.io/badge/Download-CV%20PDF-blue?style=for-the-badge&logo=adobe-acrobat-reader)](https://github.com/Darainer/CV_Ryan_Matthews/raw/main/main.pdf)

**Engineering leader with 10+ years in autonomous systems and robotics.** I define product direction in emerging technology spaces and guide multidisciplinary teams to deliver scalable autonomy platforms at commercial quality. I bring together deep technical expertise across perception, AI, and software platforms with the ability to align customer needs, technical feasibility, and strategic priorities. Focused on enabling the next wave of AI-augmented autonomy.

---

## Contact

- 🔗 [LinkedIn](https://linkedin.com/in/ryan-j-matthews)
- 📧 [ryanjmatthews@gmail.com](mailto:ryanjmatthews@gmail.com)
- 📱 +49 176 3459 9887
- 💻 [GitHub: @Darainer](https://github.com/Darainer)
- 🌐 [ryan-matthews.com](http://ryan-matthews.com/)

---

## Work Experience

### **Luminar Technologies** | *Oct 2020 – Present*

#### Program Lead – Perception Software | *Oct 2024 – Present*
- Leading commercial and technical aspects of Luminar's flagship perception software program, guiding delivery toward a vehicle production launch.
- Managing cross-functional teams to ensure alignment between product vision, technical execution, and customer requirements.

#### ADAS Solution Architect | *Jul 2022 – Oct 2024*
- Provided technical leadership in growing software department across R&D and productization of software, SoC, and lidar systems for ADAS/AD.
- Defined product and architecture boundaries to align the core platform with customer needs.
- Focused teams on commercially valuable functionality that resonated with OEMs, driving adoption and reuse.

#### Product Lead – Active Safety Software | *Oct 2020 – Sep 2022*
- Defined roadmap for a lidar-first approach to active safety and ADAS.
- Built and mentored a new software team, scaling safety-critical expertise from scratch.
- Delivered customer-facing demos (CES, IAA) that secured OEM interest and established Luminar's software credibility.

---

### **Samsung Electronics**: Product Owner – ADAS | *Apr 2019 – Oct 2020*
- Led camera-only AEB NCAP 2022+ product development using Samsung SoC as the first safety-critical application.
- Directed algorithm development and SoC design to meet application performance and safety standards.

---

### **Zenuity (Volvo / Autoliv JV)**: Senior Software Engineer | *Mar 2018 – Apr 2019*
- Developed ADAS functions and algorithms for highway automation features (L1–L3).
- Introduced KPI-driven, data-based software validation on global datasets.

---

### **Autoliv Electronics**: Technical Lead – ADAS Functions | *Oct 2016 – Mar 2018*
- Led ADAS algorithm R&D for camera-only highway cruising functions.
- Delivered successful OEM demos resulting in major program wins.

---

### **TESIS DYNAware**: Technical Lead / Simulation Specialist | *Apr 2011 – Sep 2016*
- Managed software projects exceeding $1M budget with teams of up to 10 developers for ADAS and hybrid drivetrain simulation.
- Delivered software solutions to major Tier 1 and OEM customers.

---

### **Australian Submarine Corporation**: Mechatronic Engineer | *Jan 2007 – Apr 2010*
- Designed and supported submarine control systems on active Royal Australian Navy submarines.

---

## Education

**B.Eng (Hons), Mechatronic Engineering** | University of Adelaide  
*First Class Honours, Dean's List, Aerospace Project Award*

**Bachelor of Economics** | University of Adelaide  
*Major in Mathematical Modelling and Statistics*

---

## Skills

- **Programming**: C, C++, Python, MATLAB/Simulink
- **Tooling & Standards**: ISO 26262, ISO 21448 (SOTIF), AUTOSAR C++14, CI/CD
- **Domains**: ADAS/AD, Perception, Lidar, AI, SoC, Product Strategy
- **Soft Skills**: Product Leadership, Technical Direction, Customer Engagement, Team Building

---

## Further Education

- **2024** – Deep Learning Specialization — [DeepLearning.AI](https://www.deeplearning.ai)
- **2023** – Creative Leadership Certificate — [MVR.DE](https://mvr.de)
- **2022** – Modern C++ Training — [Klaus Iglberger](https://klaus-iglberger.com)
- **2020** – Professional Scrum Product Owner — [Scrum.org](https://scrum.org)
- **2017** – Certified Scrum Master — [Scrum Alliance](https://scrumalliance.org)
- **2014** – Project Management Professional (PMP®) — [PMI](https://www.pmi.org)

---

## Languages

- **English**: Native
- **German**: Full Professional

---

## Repository Info

This repository contains the LaTeX source for my CV. It builds automatically using GitHub Actions on every push to `main`. You can find the compiled PDF in the Actions tab under Artifacts, or view [main_alt.pdf](main_alt.pdf).
